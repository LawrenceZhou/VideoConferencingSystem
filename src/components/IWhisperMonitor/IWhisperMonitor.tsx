...
export default function IWhisperMonitor() {
  ...
  return (
    <div className={classes.root}>
      ...
      <Box>
        {whisperInstanceList.map((whisperInstance, idx) => {
          ...
          const styleFrom = styleSheet.find(s => s.identity === whisperInstance.from)!;
          const styleTo = styleSheet.find(s => s.identity === whisperInstance.to)!;
          ...
          return (
            <Box
              ...
              align="center"
              background={
                whisperInstance.property === 'N/A'
                  ? 'status-unknown'
                  : whisperInstance.property === 'OK'
                  ? 'status-ok'
                  : 'status-critical'
              }
              direction="row"
            >
              ...
              {Array.isArray(nameTable) && (
                <Text size="small">
                  {nameTable!.find(n => n.role === whisperInstance.from)!.name +
                    ' whispers to ' +
                    nameTable!.find(n => n.role === whisperInstance.to)!.name}
                </Text>
              )}
            </Box>
          );
