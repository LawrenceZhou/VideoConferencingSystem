...
export declare abstract class BackgroundProcessor extends Processor {
   ...
   */
  get maskPersonBlurRadius(): number;
  /**
   * Set a new blur radius to be used when smoothing out the edges of the person's mask.
   */
  set maskPersonBlurRadius(radius: number);
  /**
   * Load the segmentation model.
   * Call this method before attaching the processor to ensure
   * video frames are processed correctly.
  ...
  //private _onResults(results: fd.Results):void;
  //private _calculateDistance(pointA:fd.NormalizedLandmark, pointB:fd.NormalizedLandmark):float;
  //private async _extraProcessFrame(image: OffscreenCanvas | HTMLCanvasElement | HTMLVideoElement):void;
}
