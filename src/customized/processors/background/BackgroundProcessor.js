...
var tv_1 = require('../../tasks-vision/vision_bundle.cjs');
var fd = require('@mediapipe/face_detection');
//var fr = require("@mediapipe/tasks-vision/FilesetResolver");
...
var BackgroundProcessor = /** @class */ (function(_super) {
  ...
  function BackgroundProcessor(options) {
    ...
    _this._pipeline = types_1.Pipeline.Canvas2D;
    // tslint:disable-next-line no-unused-variable
    ...
    _this._inputContext = _this._inputCanvas.getContext('2d', { alpha: true });
    _this._maskCanvas =
      typeof window.OffscreenCanvas !== 'undefined'
        ? new window.OffscreenCanvas(1, 1)
        : document.createElement('canvas');
    _this._maskContext = _this._maskCanvas.getContext('2d', { alpha: true });
    _this._masks = [];
    _this._maskPersonBlurRadius = 0;
    _this.maskPersonBlurRadius = 0;
    _this.faceDetector = null;
    return _this;
  }
  ...
  Object.defineProperty(BackgroundProcessor.prototype, 'maskPersonBlurRadius', {
    /**
     * The current blur radius when smoothing out the edges of the person's mask.
     */
    get: function() {
      return this._maskPersonBlurRadius;
    },
    /**
     * Set a new blur radius to be used when smoothing out the edges of the person's mask.
     */
    set: function(radius) {
      if (typeof radius !== 'number' || radius < 0) {
        console.warn('Valid mask blur radius not found. Using 0 as default.');
        radius = 0;
      }
      this._maskPersonBlurRadius = radius;
    },
    enumerable: false,
    configurable: true,
  });
      ...
      return __generator(this, function(_b) {
        switch (_b.label) {
          case 0:
            return [
              4 /*yield*/,
              Promise.all([this._loadTwilioTfLite(), fetch(this._assetsPath + constants_1.MODEL_NAME)]),
            ];
          case 1:
            _a = _b.sent();
            tflite = _a[0];
  ...
  BackgroundProcessor.prototype.loadFaceDetectionModel = async function() {
    //const filesetResolver = new tv.FilesetResolver();
    //if(filesetResolver){
    //tv.FilesetResolver.forVisionTasks()
    //wasmFileset wf = FilesetResolver.forVisionTasks();
    //let ff = new tv.FilesetResolver();
    /*const mpFaceDetection = fd;
              this.faceDetection = new mpFaceDetection.FaceDetection({ locateFile: (file) => {
        return `https://cdn.jsdelivr.net/npm/@mediapipe/face_detection@0.4/${file}`;
    } });
faceDetection.onResults(onResults);*/
    this.faceDetection = new fd.FaceDetection({
      locateFile: file => {
        //return `https://cdn.jsdelivr.net/npm/@mediapipe/face_detection@0.4/${file}`;
        return `@mediapipe/face_detection/face_detection_short_range.tflite`;
      },
    });
    ...
    this.faceDetection.onResults(onResultsx);
    //let F=tv.Fi FilesetResolver;
    //const vision =  await F.forVisionTasks("@mediapipe/tasks-vision/wasm");
    //const vision =  await tv.FilesetResolver.forVisionTasks("@mediapipe/tasks-vision/wasm");

    //this.faceDetector = await tv.FaceDetector.createFromOptions(vision, {
    //baseOptions: {
      ...
      return __generator(this, function(_d) {
        switch (_d.label) {
          case 0:
            ...
            _c = this._inferenceDimensions;
            inferenceWidth = _c.width;
            inferenceHeight = _c.height;
            inputFrame = inputFrameBuffer;
            captureWidth = inputFrame.width;
            captureHeight = inputFrame.height;
            ...
            if (this._outputCanvas !== outputFrameBuffer) {
              ...
              this._outputContext = this._outputCanvas.getContext(
                this._pipeline === types_1.Pipeline.Canvas2D ? '2d' : 'webgl2'
              );
          ...
          case 3:
            ...
            /*if(this.faceDetection) {
                        ...
                        if(this.faceDetector) {
                            ...
                            const detections = this.faceDetector.detect(inputFrame).detections;
                            if(detections && detections.length > 0){
                                if(detections[0].keypoints && detections[0].keypoints.length>0) {
                                    var distance = calculateDistance(detections[0].keypoints[0], detections[0].keypoints[1]);
                                    if (distance > 0.2){
                                        this.maskPersonBlurRadius = 10;
                                    }else if (distance <= 0.2 && distance >=0.15) {
                                        this.maskPersonBlurRadius = 5;
                                    }else {
                                        this.maskPersonBlurRadius=0;
                                    }
                                }
                            }
            ...
            ctx.filter = 'blur(' + this._maskPersonBlurRadius + 'px)';
            //ctx.filter = 'none';
            ...
            _d.label = 4;
            if (this.faceDetection) {
              console.log(this.inputFrame);
              processaframe(ctx.getImageData(inputFrame, 0, 0, captureWidth, captureHeight));
            }
          case 4:
  ...
  const processaframe = async function(frame) {
    if (this.faceDetection) {
      await this.faceDetection.send({ input: frame });
    }
  };
  ...
  BackgroundProcessor.prototype._applyAlpha = function(imageData) {
    ...
    var _loop_1 = function(i) {
      var w =
        this_1._masks.reduce(function(sum, mask, j) {
  ...
  BackgroundProcessor.prototype._createWebGL2Pipeline = function(
    inputFrame,
    captureWidth,
    captureHeight,
    inferenceWidth,
  ...
  BackgroundProcessor.prototype._loadTwilioTfLite = function() {
    return __awaiter(this, void 0, void 0, function() {
      ...
      return __generator(this, function(_b) {
        switch (_b.label) {
          case 0:
            return [4 /*yield*/, this._loadJs(this._assetsPath + constants_1.TFLITE_SIMD_LOADER_NAME)];
          case 1:
            _b.sent();
            _b.label = 2;
          case 2:
